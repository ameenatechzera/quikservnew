...
class SalesReportRepositoryImpl implements SalesReportRepository {
  ...
  @override
  ResultFuture<SalesReportResult> fetchSalesReport(FetchReportRequest request) async {
    try {
  ...
  @override
  ResultFuture<SalesDetailsByMasterIdResult> fetchSalesDetailsByMasterId(FetchSalesDetailsRequest request) async {
    try {
      final result = await remoteDataSource.fetchSalesDetailsByMasterId(request);
      return Right(result);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMessageModel.statusMessage));
    } on DioError catch (failure) {
