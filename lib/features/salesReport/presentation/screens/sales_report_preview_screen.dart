                      ...
                      return BlocBuilder<SalesReportCubit, SlesReportState>(
                        builder: (context, state) {
                          if (state is SalesDetailsSuccess) {
                            ...
                            i++) {
                              totalQty = totalQty +
                                  double.parse(saleList
                                      .first.salesDetails[i].qty
                                      .toString());
                              String truncated = totalQty.toStringAsFixed(
                                  get_decimalpoints()); // Keeps one decimal place
                              print('truncated $truncated');
                              _totalQtyController.text = truncated.toString();
                            }
                            st_GrandTotal = '0';
                            try {
                              double dbltax = double.parse(state
                                  .response.salesMaster!.vatAmount
                                  .toString());
                              double dblsubTotal = double.parse(state
                                  .response.salesMaster!.subTotal
                                  .toString());
                              double dblGrandTotal = dbltax + dblsubTotal;
                              st_GrandTotal = dblGrandTotal
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            try {
                              double dbl_SubTotal = double.parse(state
                                  .response.salesMaster!.subTotal
                                  .toString());
                              st_SubTotal = dbl_SubTotal
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            try {
                              double dbl_TaxableAmt = double.parse(state
                                  .response.salesMaster!.subTotal
                                  .toString());
                              st_TaxableAmt = dbl_TaxableAmt
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            try {
                              double dbl_TotalTax = double.parse(state
                                  .response.salesMaster!.vatAmount
                                  .toString());
                              st_TotalTax = dbl_TotalTax
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            if (st_vatType == 'GST') {
                              double dbl_TotalTax = double.parse(state
                            ...
                            try {
                              double dbl_TotalDisc = double.parse(state
                                  .response
                                  .salesMaster!
                                  .discountAmount
                                  .toString());
                              st_totalDisc = dbl_TotalDisc
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            try {
                            ...
                            try {
                              double dbl_NetInvAmt = double.parse(state
                                  .response.salesMaster!.grandTotal
                                  .toString());
                              st_netInvAmt = dbl_NetInvAmt
                                  .toStringAsFixed(get_decimalpoints());
                            } catch (_) {}
                            ...
                            //     .toString();
                            String st_custName = state
                                .response.salesMaster!.ledgerName
                                .toString();
                            ...
                            listHeight = 70.00 *
                                state.response.salesDetails.length;
                            return Container(
                              color:
                              //appThemeOrange,
                              appThemegrayColors,
  ...
  Future<void> fetchDetails(String stMasterID) async {
    ...
    context.read<SalesReportCubit>().fetchSalesDetailsByMasterId(
      FetchSalesDetailsRequest(
        ...
        branchId: st_branchIdPref, SalesMasterId: stMasterID,
      ),
    );
  }
