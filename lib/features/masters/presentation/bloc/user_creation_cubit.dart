...
class UserCreationCubit extends Cubit<UserCreationState> {
  ...
  UserCreationCubit({ required FetchUserTypesUseCase fetchUserTypesUseCase ,
    required SaveUserUseCase saveUserTypesUseCase , required FetchSupplierListUseCase fetchSupplierListUseCase ,
    required FetchCashierListUseCase fetchCashierListUseCase}) :
        _fetchUserTypesUseCase = fetchUserTypesUseCase,_saveUserTypesUseCase = saveUserTypesUseCase,
        _fetchSupplierListUseCase = fetchSupplierListUseCase, _fetchCashierListUseCase = fetchCashierListUseCase , super(UserCreationInitial());
  // --------------------- API Fetch ---------------------
    ...
    try {
      ...
      result.fold(
          ...
          emit(FetchUserTypesFailure(failure.message));
            ...
            (userTypeResponse) {
          emit(FetchUserTypesFromCreationLoaded(userTypes: userTypeResponse.details));
        },
      );
    } catch (e, stacktrace) {
      // Handle unexpected exceptions
  ...
  Future<void> fetchSupplierList() async {
    ...
    try {
      ...
      result.fold(
          ...
          emit(FetchSupplierListLoaded(supplierList: supplierListResponse.details));
        },
      );
    } catch (e, stacktrace) {
      // Handle unexpected exceptions
