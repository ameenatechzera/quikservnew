...
import 'package:quikservnew/features/sale/data/models/loyalty_search_model.dart';
...
abstract class SalesRemoteDataSource {
  ...
  Future<LoyaltySearchModel> fetchLoyaltyDetailsBySearch(LoyaltySearchRequest request);
...
class SalesRemoteDataSourceImpl implements SalesRemoteDataSource {
  ...
  @override
  Future<LoyaltySearchModel> fetchLoyaltyDetailsBySearch(LoyaltySearchRequest request) async {
    try {
      final baseUrl = await SharedPreferenceHelper().getBaseUrl();
      if (baseUrl == null || baseUrl.isEmpty) {
        throw Exception("Base URL not set");
      }
      final url = ApiConstants.fetchLoyaltySearchCustomerPath(baseUrl);
      print('url $url');
      final dbName = await SharedPreferenceHelper().getDatabaseName();
      final token = await SharedPreferenceHelper().getToken() ?? "";
      if (token.isEmpty) throw Exception("Token missing! Please login again.");
      ...
      print('URL $url');
      if (response.statusCode == 200 || response.statusCode == 201) {
        return LoyaltySearchModel.fromJson(response.data);
      } else {
        throw ServerException(
          errorMessageModel: ErrorMessageModel.fromJson(response.data),
        );
