...
class SalesCountGraphResult extends Equatable {
  ...
  factory SalesCountGraphResult.fromJson(Map<String, dynamic> json) {
    return SalesCountGraphResult(
      ...
      data: json["data"] == null
          ? []
          : List<SaleCountGraphList>.from(
              json["data"]!.map((x) => SaleCountGraphList.fromJson(x)),
            ),
    );
  }
  ...
  @override
  List<Object?> get props => [status, error, period, data];
}

class SaleCountGraphList extends Equatable {
  const SaleCountGraphList({required this.name, required this.value});
  ...
  SaleCountGraphList copyWith({String? name, String? value}) {
